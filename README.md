# Appcues-Mobile-Demo-App
A repo and guide to setting up a demo version of the Appcues mobile app
...
Push notifications cannot be tested unless you set up your own Apple Developer license
Any time Xcode has an update, you will need to reinstall the app manually
What the app does
Screen	Purpose
...
Click AppcuesTestApp in the left sidebar under TARGETS
Go to the Signing & Capabilities tab
Under Team, select your personal Apple ID
If you see a signing error, click Try Again or Register Device
6. Run the app on your iPhone
Connect your iPhone via USB
Select your iPhone from the device dropdown at the top of Xcode
Hit the Play button
The first time, your iPhone may ask you to trust the developer — go to Settings → General → VPN & Device Management on your phone and trust your Apple ID
Running after initial setup
Once set up, you don't need to rebuild in Xcode every time. Just:

Open Terminal and run npm start in the project folder
...
Troubleshooting
App says "no longer available" on my phone You built for a different target (e.g. Mac). Select your iPhone in Xcode and rebuild.

Pod install fails with a Unicode error Run LANG=en_US.UTF-8 pod install instead from the ios folder.

Signing error in Xcode Make sure you're signed into Xcode with your Apple ID under Xcode → Settings → Accounts.

App builds but crashes immediately Try a clean build: In Xcode go to Product → Clean Build Folder (Cmd+Shift+K), then hit Play again.
